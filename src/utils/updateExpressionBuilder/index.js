...
function getVersionLockExpression({
  ...
  if (newVersion === undefined) {
    if (currentVersion === undefined || currentVersion === null) {
      ...
      throw new Error(
        `Invalid arguments. Must specify [newVersion] for non-numeric currentVersion: [${currentVersion}]`
      );
    }
  }
  const _original = {};
...
function version({
  ...
  const expectedVersionNode = alias(
    ...
    expectedVersion !== undefined
      ...
      : undefined, // else: no need for ExpressionAttributeValues
    {
  ...
  if (currentVersion !== undefined || currentVersion === null) {
    ...
    delete conditionExpression.ExpressionAttributeValues[
      expectedVersionNode.value
    ]; //value is aliasedValue
  }
...
function getUpdateExpression({
  ...
  const { SET, REMOVE, DELETE } = partitionedDiff(
    original,
    modified,
    orphans,
    supportSets
  ...
  updateExpression.UpdateExpression = [setExp, removeExp, deleteExp].reduce(
    (acc, value) => {
...
function checkLimit(name, maxLen = maxAttrNameLen) {
  if (name.length > maxLen)
    throw new Error(
      `Attribute name: [${name}] exceeds DynamoDB limit of [${maxLen}] `
    );
}

function truncate(
  name,
  maxLen = maxAttrNameLen - 1,
  aliasContext = { truncatedAliasCounter: 1 }
) {
  if (name.length <= maxLen) {
    return name;
...
function alias(node, nameMap, valueMap, aliasContext = {}) {
  ...
  const pathParts = parts
    ...
    .map((part) => {
      ...
      if (regex.isInvalidIdentifierName.test(part)) {
        ...
        checkLimit(attrName);
        attrNameAlias = `#${truncate(
          _.camelCase([prefix, attrName]),
          maxAttrNameLen - 1,
          aliasContext
        )}`; // #xY
        pathPart = attrNameAlias; // #xY
      } else if (regex.isNumericSubscript$.test(part)) {
        const [whole, _attrName, subscript] =
          regex.isNumericSubscript$.exec(part); // relatedItems[1]
  ...
  let { value } = node;
  if (valueMap) {
    const valueAlias = `:${truncate(
      _.camelCase([prefix, ...parts]),
      maxAttrNameLen - 1,
      aliasContext
    )}`;
...
function partitionedDiff(
  ...
  const { ADD, DELETE, SET } = diff(original, modified, orphans);
  const [_DELETE, _REMOVE] = supportSets
    ? _.partition(
        DELETE,
        (node) =>
          regex.isNumericSubscript$.test(node.path) &&
          (_.isNumber(node.value) || _.isString(node.value)) // @TODO: Support Node Buffer and/or ArrayBuffer sets?
      )
    : [[], DELETE];
...
function sortBy(sortBy, mapping = (v) => v) {
  return (a, b) =>
    +(mapping(a[sortBy]) > mapping(b[sortBy])) ||
    +(mapping(a[sortBy]) === mapping(b[sortBy])) - 1;
}
...
function leafNodes(nodes, sort = false) {
  ...
  return nodes.reduce((acc, node, index, arr) => {
    if (index < arr.length - 1 && isParentOf(node.path, arr[index + 1].path))
      return acc; // skip parent node
    acc.push(node);
    return acc;
  }, []);
}
